...
public class MoodHistoryController {
  ...
  private String currentUserIdOrNull() {
    ...
    return LoginController.getUser().getId().toString();
  }
  ...
  private void reloadMasterFromDb() {
    try {
      final String currentUserId = currentUserIdOrNull();
      ...
      // Load everything
      final List<MoodHistoryItem> data = new MoodEntryDao().findHistory(currentUserId,
          null, "ALL");
  ...
  private void onEdit(final MoodCardVM m) {
    try {
      ...
      final MoodHistoryItem it = dao.findById(m.id, currentUserId);
      if (it == null)
        return;
  ...
  private void onDelete(final MoodCardVM m) {
    ...
    if (res.isPresent() && res.get() == javafx.scene.control.ButtonType.OK) {
      try {
        ...
        final boolean ok = new MoodEntryDao().delete(m.id, currentUserId);
