...
public class StepEmotionsController {
  ...
  public void initialize() {
    if (resources == null) {
      throw new IllegalStateException(
          "ResourceBundle not injected for StepEmotions. Load StepEmotions.fxml with a bundle.");
    }

    for (final String code : EMOTION_CODES) {
  ...
  public void setSelectedEmotions(final List<String> codes) {
    prefilling = true;
    try {
      selected.clear();
      ...
      if (codes != null) {
        for (final String c : codes) {
          if (c == null)
            continue;
          final String cleaned = c.trim().toUpperCase(Locale.ROOT);
          if (!cleaned.isEmpty())
  ...
  private void updateLimitUI() {
    ...
    if (left <= 0) {
      ...
      final String pattern = resources.getString("emotions.limit.max");
      limitLabel.setText(java.text.MessageFormat.format(pattern, MAX_SELECTED));
    } else {
      // "1 selection left." / "2 selections left."
      final String one = resources.getString("emotions.limit.left.one");
      final String many = resources.getString("emotions.limit.left.many");
      limitLabel.setText(java.text.MessageFormat.format(left == 1 ? one : many, left));
    }
  }
} // StepEmotionsController class
