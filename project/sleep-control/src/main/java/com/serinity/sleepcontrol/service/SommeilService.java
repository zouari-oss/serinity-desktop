...
public class SommeilService {
    ...
    /**
     * Score global 0→100 :
     *   40% score qualité moyen
     *   30% durée (idéal = 8h)
     *   15% absence d'interruptions (max 5)
     *   15% % nuits durée optimale
     */
    ...
    /** Ratio nuits Bonne/Excellente vs total → 0.0 à 1.0 */
    public double calculerIndexResilience() throws SQLException {
        List<Sommeil> s = listerTous();
        if (s.isEmpty()) return 0;
        long positives = s.stream()
                .filter(n -> "Excellente".equals(n.getQualite())
                        || "Bonne".equals(n.getQualite()))
                .count();
    ...
    // ═══════════════════════════════════════════════════════════
    //  CLASSEMENT PAR NIVEAU  (avancé)
    // ═══════════════════════════════════════════════════════════

    /**
     * Classe les nuits en 3 niveaux selon le score qualité.
     * @return Map avec clés "🟢 BON", "🟡 MOYEN", "🔴 MAUVAIS"
     */
    ...
    // ═══════════════════════════════════════════════════════════
    //  DÉTECTION D'ANOMALIES  (avancé)
    // ═══════════════════════════════════════════════════════════

    /**
     * Nuits statistiquement anormales :
     * durée < moyenne - 2×écart-type  OU  interruptions > moyenne + 2×écart-type
     */
    public List<Sommeil> detecterAnomalies() throws SQLException {
        ...
        double varDuree  = s.stream().mapToDouble(n -> Math.pow(n.getDureeSommeil() - moyDuree, 2)).average().orElse(0);
        double sdDuree   = Math.sqrt(varDuree);

        double moyInter  = s.stream().mapToInt(Sommeil::getInterruptions).average().orElse(0);
        double varInter  = s.stream().mapToDouble(n -> Math.pow(n.getInterruptions() - moyInter, 2)).average().orElse(0);
        double sdInter   = Math.sqrt(varInter);
    ...
    // ═══════════════════════════════════════════════════════════
    //  RÉGULARITÉ DES HORAIRES  (avancé)
    // ═══════════════════════════════════════════════════════════

    /**
     * Calcule l'écart-type des heures de coucher (en minutes depuis minuit).
     * Plus c'est bas, plus les horaires sont réguliers.
     * @return écart-type en minutes
     */
    public double calculerRegulariteHoraires() throws SQLException {
        ...
        List<Integer> minutes = s.stream()
                .map(n -> {
                    LocalTime h = n.getHeureCoucher();
                    int m = h.getHour() * 60 + h.getMinute();
                    // normalisation : heure > 20h00 = coucher normal, sinon lendemain matin
        ...
        double variance = minutes.stream()
                .mapToDouble(m -> Math.pow(m - moy, 2)).average().orElse(0);
    ...
    // ═══════════════════════════════════════════════════════════
    //  DETTE DE SOMMEIL  (avancé)
    // ═══════════════════════════════════════════════════════════

    /**
     * Dette de sommeil sur les 7 derniers jours.
     * Formule : (8h × 7 nuits) − durée totale dormie.
     * Valeur positive = dette, négative = excès.
     * @return dette en heures
     */
    public double calculerDetteSommeil() throws SQLException {
        ...
        if (s.isEmpty()) return 56.0; // 8h × 7 nuits manquées
    ...
    // ═══════════════════════════════════════════════════════════
    //  EFFICACITÉ DU SOMMEIL  (avancé)
    // ═══════════════════════════════════════════════════════════

    /**
     * Efficacité = (durée dormie / temps passé au lit) × 100
     * Suppose temps au lit = durée calculée (heureCoucher → heureReveil).
     * Une efficacité ≥ 85% est considérée bonne.
     * @return efficacité en % (0-100)
     */
    public double calculerEfficaciteSommeil(Sommeil sommeil) {
        ...
        // On estime le temps réellement dormé : tempsLit - 15min par interruption
        double estimeDormi = tempsLit - (sommeil.getInterruptions() * 0.25);
        double efficacite  = (estimeDormi / tempsLit) * 100;
    ...
    public double calculerEfficaciteMoyenne() throws SQLException {
        ...
        if (s.isEmpty()) return 0;
        return s.stream()
                .mapToDouble(this::calculerEfficaciteSommeil)
                .average().orElse(0);
    }
    ...
    public Map<String, String> identifierTendances(LocalDate debut, LocalDate fin)
        ...
        if (s.size() < 3) {
            tendances.put("message", "Pas assez de données");
            return tendances;
        }

        List<Double> durees = s.stream().map(Sommeil::getDureeSommeil).collect(Collectors.toList());
        List<Double> scores = s.stream().map(n -> (double) n.calculerScoreQualite()).collect(Collectors.toList());
        List<Double> inters = s.stream().map(n -> (double) n.getInterruptions()).collect(Collectors.toList());

        tendances.put("duree",          tendance(durees, true));
        tendances.put("qualite",        tendance(scores, true));
        tendances.put("interruptions",  tendance(inters, false)); // décroissant = bien
        return tendances;
    }
    ...
    // ═══════════════════════════════════════════════════════════
    //  PROFIL CHRONOBIOLOGIQUE  (avancé)
    // ═══════════════════════════════════════════════════════════

    /**
     * Détermine si l'utilisateur est plutôt "couche-tôt" ou "couche-tard"
     * selon la médiane des heures de coucher.
     */
    public String determinerProfilChronobiologique() throws SQLException {
        ...
        List<Integer> minutes = s.stream()
                .map(n -> {
                    ...
                    return m < 12 * 60 ? m + 24 * 60 : m; // normalisation
                })
                .sorted()
                .collect(Collectors.toList());
    ...
    // ═══════════════════════════════════════════════════════════
    //  RECOMMANDATIONS AVANCÉES  (avancé)
    // ═══════════════════════════════════════════════════════════
    ...
    public List<String> genererRecommandations() throws SQLException {
        ...
        if (s.isEmpty()) {
            rec.add("Commencez à enregistrer vos nuits de sommeil");
            return rec;
        }
        ...
        // Durée
        if      (dureeMoy < 7) {
            rec.add("Dormez au moins 7h par nuit — visez 8h");
            rec.add("Couchez-vous 30 min plus tôt chaque soir");
        } else if (dureeMoy > 9) {
            rec.add("Durée excessive : peut indiquer de la fatigue chronique");
        }

        // Interruptions
        if (interMoy > 2) {
            rec.add("Réduisez les sources de bruit et de lumière la nuit");
            rec.add("Maintenez 18–20°C dans votre chambre");
            rec.add("Évitez les écrans 1h avant le coucher");
        }

        // Dette
        if (dette > 5) {
            rec.add("Dette de sommeil élevée : planifiez une nuit de récupération");
        }

        // Régularité
        if (regularite > 45) {
            rec.add("Fixez des horaires de coucher et réveil fixes, même le week-end");
        }

        // Efficacité
        if (efficacite < 80) {
            rec.add("Efficacité faible : levez-vous si vous ne dormez pas après 20 min");
            rec.add("Pratiquez une relaxation musculaire progressive avant de dormir");
        }

        if (rec.isEmpty())
            rec.add("Votre sommeil est de bonne qualité — continuez ainsi !");
    ...
    /**
     * Génère une liste de phrases clés sur le profil de sommeil.
     */
    public List<String> obtenirInsights() throws SQLException {
        ...
        if (s.isEmpty()) {
            insights.add("Aucune nuit enregistrée pour le moment.");
            return insights;
        }
        ...
        insights.add("⚡ Efficacité moyenne : "
                + String.format("%.1f%%", efficacite) + " — "
                + libelleEfficacite(efficacite));
        insights.add("💪 Résilience : "
                + String.format("%.0f%%", resilience * 100) + " — "
                + libelleResilience(resilience));
        insights.add(determinerProfilChronobiologique());

        Sommeil meilleure = trouverMeilleurSommeil();
        if (meilleure != null)
            insights.add("🌟 Meilleure nuit : " + meilleure.getDateNuit()
                    + " (score " + meilleure.calculerScoreQualite() + "/100)");

        Map<String, Long> qualites = compterParQualite();
        qualites.entrySet().stream()
                .max(Map.Entry.comparingByValue())
                .ifPresent(e -> insights.add(
                        "📊 Qualité dominante : " + e.getKey() + " (" + e.getValue() + " nuits)"));

        return insights;
    }

    // ═══════════════════════════════════════════════════════════
    //  RAPPORT DÉTAILLÉ  (avancé)
    // ═══════════════════════════════════════════════════════════
    ...
    public String genererRapportDetaille(Sommeil sommeil) {
        ...
        sb.append("── Analyse ──────────────────────────\n");
        sb.append(analyserQualiteSommeil(sommeil)).append("\n");
    ...
    public String genererRapportGlobal() throws SQLException {
        ...
        sb.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        sb.append("  RAPPORT GLOBAL DU SOMMEIL\n");
        sb.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n");
        ...
        compterParQualite().forEach((q, n) ->
                sb.append("  ").append(q).append(" : ").append(n).append(" nuits\n"));
