...
public class FavoriteDao {
    ...
    public int insert(Favorite f) throws SQLException {
        String sql = "INSERT INTO exercice_favorite (user_id, favorite_type, item_id) VALUES (?, ?, ?)";
    ...
    public void delete(int id) throws SQLException {
        String sql = "DELETE FROM exercice_favorite WHERE id=?";
    ...
    public void deleteByUserTypeAndItem(int userId, String favoriteType, int itemId) throws SQLException {
        String sql = "DELETE FROM exercice_favorite WHERE user_id=? AND favorite_type=? AND item_id=?";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ps.setInt(1, userId);
    ...
    public boolean exists(int userId, String favoriteType, int itemId) throws SQLException {
        String sql = "SELECT 1 FROM exercice_favorite WHERE user_id=? AND favorite_type=? AND item_id=? LIMIT 1";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ps.setInt(1, userId);
            ps.setString(2, favoriteType);
            ps.setInt(3, itemId);
    ...
    public List<Favorite> findByUserId(int userId) throws SQLException {
        String sql = "SELECT * FROM exercice_favorite WHERE user_id=? ORDER BY id DESC";
