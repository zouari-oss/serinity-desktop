package com.serinity.forumcontrol.service;
...
import com.serinity.forumcontrol.config.DbConnection;
...
public class ServiceThread implements Services<Thread> {
    ...
    public void add(Thread thread) {
        if ((thread.getUserId() == null || thread.getUserId().isEmpty()) && LoginController.getUser() != null) {
            thread.setUserId(LoginController.getUser().getId().toString());
        }
...
""";
    ...
    public boolean istherapist(String userId){
        ...
        try (PreparedStatement ps =
            ...
            if (rs.next()) {
                if(rs.getString("role").equals("therapist")){return true;}
            }
