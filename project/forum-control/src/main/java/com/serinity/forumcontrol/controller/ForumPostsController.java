...
import com.serinity.forumcontrol.model.ThreadStatus;
import com.serinity.forumcontrol.model.ThreadType;
import com.serinity.forumcontrol.service.ServiceCategory;
import com.serinity.forumcontrol.service.ServiceNotification;
import com.serinity.forumcontrol.service.ServicePostInteraction;
...
import com.serinity.forumcontrol.model.Thread;
import com.serinity.forumcontrol.service.ServiceThread;
...
public class ForumPostsController {
    String currentUserId = (LoginController.getUser() != null && LoginController.getUser().getId() != null) ? LoginController.getUser().getId().toString() : "";
        ...
        try {
            ...
            showAlert("Error Loading View",
                    "Could not load New Thread form.\n\n" +
                            "Error: " + e.getMessage() + "\n\n" +
                            "Check that AddThread.fxml is at: /fxml/AddThread.fxml",
    ...
    public void loadThreads() {
        ...
        try {
            ...
            List<Thread> threads = service.getAll(currentUserId).stream().filter(t-> t.getStatus()!= ThreadStatus.ARCHIVED).toList();
    ...
    public void loadMyThreads() {
        ...
        try {
            cardsContainer.getChildren().clear();
            ServiceThread service = new ServiceThread();
            List<Thread> threads = service.getAll().stream().filter(t-> t.getUserId().equalsIgnoreCase(currentUserId)).filter(t-> t.getStatus()!= ThreadStatus.ARCHIVED).toList();
            currentThreads = threads;
            displayThreads(threads);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void loadArchivedThreads() {
        ...
        try {
            cardsContainer.getChildren().clear();
            ServiceThread service = new ServiceThread();
            List<Thread> threads = service.getAll().stream().filter(t-> (t.getUserId().equalsIgnoreCase(currentUserId)) && t.getStatus()== ThreadStatus.ARCHIVED).toList();
            currentThreads = threads;
            displayThreads(threads);
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ...
            for (Thread t : threads) {
                FXMLLoader loader = new FXMLLoader(
                        getClass().getResource("/fxml/ThreadCard.fxml"));
    ...
    private void updateNotificationBadge() {
        int unseenCount = notificationService.getUnseenCount("1");
    ...
    private void onNotifications() {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/fxml/NotificationsPanel.fxml")
            );
