...
import com.serinity.forumcontrol.service.ServiceReply;
...
public class ThreadCardController {
        ...
        try {
            FXMLLoader loader =
                    new FXMLLoader(getClass()
                            .getResource("/fxml/ThreadDetailView.fxml"));
    ...
    private void configureMenu() {
        try {
            ...
            boolean isAdmin = service.istherapist(currentUserId);
    ...
    private void handleEdit(ActionEvent event) {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/fxml/AddThread.fxml"));
    ...
    private BorderPane findBorderPane() {
        javafx.scene.Node node = rootCard;
        while (node != null) {
    ...
    private void refreshCurrentPage(int k) {
        try {
            // Find the ForumPostsController that's already loaded in the scene
            BorderPane borderPane = findBorderPane();
            if (borderPane == null) return;

            // Get the center node's controller - it's already ForumPostsView
            javafx.scene.Node center = borderPane.getCenter();
            if (center == null) return;

            ForumPostsController controller = (ForumPostsController) center.getUserData();
            if (controller != null) {
                if (k == 1) controller.loadThreads();
                else if (k == 2) controller.loadMyThreads();
                else controller.loadArchivedThreads();
            }
        } catch (Exception e) {
