package com.serinity.accesscontrol.controller.backoffice;
...
public class BackofficeAppController {
  ...
  // Mood
  @FXML
  private TextField tfMoodUser;
  @FXML
  private Button btnMoodRefresh;
  @FXML
  private Button btnMoodDeleteSelected;
  @FXML
  private Button btnMoodClearSelection;
  @FXML
  private VBox moodCardsBox;

  // Journal
  @FXML
  private TextField tfJournalUser;
  @FXML
  private Button btnJournalRefresh;
  @FXML
  private Button btnJournalDeleteSelected;
  @FXML
  private Button btnJournalClearSelection;
  @FXML
  private VBox journalCardsBox;
  ...
  private void deleteSingleMood(long id, Node cardNode) {
    if (!confirm("Delete mood entry " + id + "?"))
      return;
  ...
  private void deleteSingleJournal(long id, Node cardNode) {
    if (!confirm("Delete journal entry " + id + "?"))
      return;
  ...
  private void deleteSelectedMoods() {
    if (selectedMoodIds.isEmpty()) {
      setStatus("No mood cards selected.");
      return;
    }
    int n = selectedMoodIds.size();
    if (!confirm("Delete " + n + " selected mood entr" + (n == 1 ? "y" : "ies") + "?"))
      return;
    ...
    runAsync(() -> {
      ...
      for (Long id : ids) {
        Node card = moodCardById.get(id);
        if (card != null)
          moodCardsBox.getChildren().remove(card);
        moodCardById.remove(id);
      }
      selectedMoodIds.clear();
      updateSelectionButtons();
  ...
  private void deleteSelectedJournals() {
    if (selectedJournalIds.isEmpty()) {
      setStatus("No journal cards selected.");
      return;
    }
    int n = selectedJournalIds.size();
    if (!confirm("Delete " + n + " selected journal entr" + (n == 1 ? "y" : "ies") + "?"))
      return;
    ...
    runAsync(() -> {
      ...
      for (Long id : ids) {
        Node card = journalCardById.get(id);
        if (card != null)
          journalCardsBox.getChildren().remove(card);
        journalCardById.remove(id);
      }
      selectedJournalIds.clear();
      updateSelectionButtons();
