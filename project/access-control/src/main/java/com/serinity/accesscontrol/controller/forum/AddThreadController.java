...
import com.serinity.accesscontrol.service.forum.ServiceImgBB;
import com.serinity.accesscontrol.service.forum.Moderate;
...
public class AddThreadController {
    ...
    public void initialize() {
        if (LoginController.getUser() != null) {
            currentUserId = LoginController.getUser().getId().toString();
        }
        setAddMode();
    ...
    private void checkAdminAndSetPinnedVisibility() {
        boolean isTherapist = LoginController.getUser() != null
                && (LoginController.getUser().getRole() == UserRole.THERAPIST
                    || LoginController.getUser().getRole() == UserRole.ADMIN);
        if (isTherapist) {
    ...
    private BorderPane findBorderPane() {
        ...
        BorderPane last = null;
        while (node != null) {
            if (node instanceof BorderPane) {
                last = (BorderPane) node;
            }
            node = node.getParent();
        }
        return last;
