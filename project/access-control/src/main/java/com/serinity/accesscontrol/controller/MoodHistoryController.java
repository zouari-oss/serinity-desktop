...
public class MoodHistoryController {
  ...
  @FXML
  private TextField searchField; // local search (no SQL)
  @FXML
  ...
  private List<MoodCardVM> getItemsForStats() {
    ...
    if (t("history.range.last7").equals(rangeLabel))
      lastDays = Integer.valueOf(7);
    else if (t("history.range.last30").equals(rangeLabel))
      lastDays = Integer.valueOf(30);

    final LocalDateTime cutoff = (lastDays == null) ? null
        : LocalDate.now().minusDays(lastDays.intValue()).atStartOfDay();

    final List<MoodCardVM> filtered = new ArrayList<MoodCardVM>();
    for (final MoodCardVM m : masterItems) {

      // range filter
      if (cutoff != null && m.dateTime.isBefore(cutoff))
        continue;
  ...
  private void reloadMasterFromDb() {
    try {
      ...
      // Load everything
      final List<MoodHistoryItem> data = new MoodEntryDao().findHistory(LoginController.getUser().getId().toString(),
          null, "ALL");
  ...
  private void onEdit(final MoodCardVM m) {
    try {
      ...
      final MoodHistoryItem it = dao.findById(m.id, LoginController.getUser().getId().toString());
      if (it == null)
        return;

      final com.serinity.accesscontrol.model.MoodEntry entry = new com.serinity.accesscontrol.model.MoodEntry();
      entry.setId(it.getId());
      entry.setUserId(LoginController.getUser().getId().toString());
  ...
  private void onDelete(final MoodCardVM m) {
    ...
    if (res.isPresent() && res.get() == javafx.scene.control.ButtonType.OK) {
      try {
        final boolean ok = new MoodEntryDao().delete(m.id, LoginController.getUser().getId().toString());
