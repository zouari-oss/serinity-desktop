 ...
 * Data source: cached MoodHistory items passed from MoodHistoryController (no
 * DB hits).
 */
public class MoodStatsPanelController {
  ...
  @FXML
  private ToggleButton tabMoment;
  @FXML
  private ToggleButton tabDay;
  @FXML
  private ToggleButton tabTotal;
  ...
  private List<MoodHistoryController.MoodCardVM> items = new ArrayList<MoodHistoryController.MoodCardVM>();
  ...
  private List<MoodHistoryController.MoodCardVM> last30Days(List<MoodHistoryController.MoodCardVM> src) {
    if (src == null)
      return new ArrayList<MoodHistoryController.MoodCardVM>();
    ...
    for (MoodHistoryController.MoodCardVM m : src) {
      if (m == null || m.dateTime == null)
        continue;
      if (!m.dateTime.isBefore(cutoff))
        out.add(m);
    }
    return out;
  }
  ...
  private void buildPie() {
    ...
    // lvls debug
    // System.out.println("Invalid levels: " + src.stream().filter(m -> m.moodLevel
    // < 1 || m.moodLevel > 5).count());
