...
public final class JournalValidation {
  ...
  private static String validateField(String value, String fieldKey, int maxLen) {
    if (value == null)
      return fieldKey + "|journal.validation.required";

    String v = value.trim();
    if (v.isEmpty())
      return fieldKey + "|journal.validation.empty";

    v = v.replace("\r\n", "\n");
    if (v.length() > maxLen)
      return fieldKey + "|journal.validation.max_len";

    for (int i = 0; i < v.length(); i++) {
      char c = v.charAt(i);
      if (FORBIDDEN.contains(c))
        return fieldKey + "|journal.validation.forbidden";
    }
    return null;
  }
