...
import com.serinity.consultationcontrol.service.DashboardService;
...
public class DashboardController {
        ...
        try{
            ...
            for (XYChart.Data<String, Number> data : series.getData()) {
                ...
                data.nodeProperty().addListener((obs, oldNode, node) -> {
                    if(node != null){
                        switch(status){
                            case "APPROUVE" -> node.setStyle("-fx-bar-fill: #4CAF50;");
                            case "REFUSE" -> node.setStyle("-fx-bar-fill: #D9534F;");
                            case "EN_ATTENTE" -> node.setStyle("-fx-bar-fill: #F4B400;");
                            default -> {
                            }
                        }
                    }
    ...
    private void loadMonthlyChart(){
        try{
            ...
            while(rs.next()){
                ...
                int total = rs.getInt("total");
                String monthName = java.time.Month.of(month).getDisplayName(TextStyle.SHORT, Locale.FRANCE);
    ...
    private void loadTopPatients(){
        try{
            ...
            XYChart.Series<String, Number> series = new XYChart.Series<>();
            series.setName("Patients frequents");
